...
{
    public static HashSet<string> Read(JsonElement slots, HashSet<string> sources, HashSet<string> discovered)
    {
        ...
        foreach (var slot in slots.EnumerateArray())
        {
            ...
            Require(sources.Contains(path) && discovered.Contains(label), "Visual-slot label lacks its asset or metadata source.");
            ...
            foreach (var member in slot.GetProperty("members").EnumerateArray())
            {
                ...
                Require(discovered.Contains(item) && discovered.Contains(metadata) && members.Add((item, metadata)),
